...
class Event {
	...
	async process(client, chain, member) {
		...
		// 	});
		// });
		await member.guild.channels.cache.get(process.env.WELCOME_CHANNEL).send({ content: `${member.user} just joined. Welcome! 🎉` });
		member.guild.channels.cache.get(process.env.WELCOME_CHANNEL).send({ content: 'https://media.giphy.com/media/T8wyBKRBrzTZJjU0eQ/giphy.gif' });

		if (!member.user.bot) {
			member.send(`Hey there and thanks for joining **${member.guild.name}**! We hope you have fun on our server!\nIn order to gain access to the rest of the server you will need to verify!`)
				.catch(() => client.logger.warn(`Couldn't DM user ${member.user.tag}, probably because they have DMs off`));
		}
	}
